...
| ```/expense``` | ```405```: method not allowed | ```405```: method not allowed | ```200```: creates a new expense <br/>OR <br/>```400```: bad request if error | ```405```: method not allowed | Cannot reassign an expense that has been commited to a group A to group B. The expense must be deleted and recreated under correct group |
